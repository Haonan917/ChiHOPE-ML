...
import pandas as pd
from models.classfication_models import test_all_models
...
def main(args):
    ...
    validation_dir_name = data_config['validation_dir_name']
    
    omics_validation_path = [os.path.join(omics_parent_dir, validation_dir_name, omics_file)
    ...
    if feature_selection_dir is not None: # otherwise, only include selected features and its omics type
        ...
        if selected_feature_path.exists():
            ...
            omics_validation_path = [omics_validation_path[i] for i in range(len(omics_validation_path)) if i not in ignored_omics_indices]
    ...
    # load omics file
    X_array, y = load_omics(omics_validation_path, test_endpoint_path, endpoint, selected_feature_lists=selected_feature_lists)
    selected_feature_lists = [omics_data.columns.to_list() for omics_data in X_array]
    X= pd.concat(X_array, axis=1)
    print(f"Shape of data: {X.shape}")
    refit_models = pickle.load(open(interim_path / 'output_refits_{}.pkl'.format(data_type), 'rb'))
    test_all_models(X, y, refit_models, save_dir=interim_path, omicsComb=data_type, random_state=str(seed))

if __name__ == '__main__':   
    # data_config = './config/cgz_main_config/cg/select_1/CLN.yaml'
    data_config = './config/cgz_main_config/cg/select_5/CLN_MTB_PRT_MIR_MET.yaml'
    # data_config = './config/cgz_main_config/cg/select_3/CLN_MTB_MET.yaml'
